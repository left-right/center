<div class="form-group field-{{ $field->type }} @if ($errors->has($field->name)) has-error @endif">
	...
	<div class="col-sm-10">
		<div class="input-group time" data-date-format="hh:mm A">
			...
			<input type="text" class="form-control time @if ($field->required) required @endif" @if (isset($row->{$field->name})) value="{{ date('g:i A', strtotime($row->{$field->name})) }}" @endif name="{{ $field->name }}" id="{{ $field->name }}">
		</div>
	</div>
</div>